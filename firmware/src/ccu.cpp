 ...
 *   2. Creates serial control commands for the U6, and receive U6 stats (other arduino on the CCU board)
...
volatile bool Initialized; // this will be 1 if it is not initiating! zero for the
                         // begining, zero it after receving Seq form PC
...
byte SigGenResponse[64]; // contains the most up to date info for the signal generator
...
bool ManualTimerStatus = false; // used to generate delay without keeping the CPU Busy
...
respondToPC(void)
{
	...
	byte outKey[64] = {54,  101, 116, 116, 117, 74, 108, 85,  101, 55,  107,
	                   97,  109, 104, 78,  97,  52, 115, 104, 112, 82,  102,
	                   103, 122, 67,  98,  106, 66, 55,  101, 114, 121, 48,
	                   68,  65,  105, 100, 74,  49, 102, 71,  57,  100, 81,
	                   103, 82,  80,  56,  55,  87, 110, 79,  50,  114, 99,
	                   97,  114, 108, 84,  110, 50, 101, 111, 50};
	bool Error = false;
	...
	if (inbytes[0] == 'S' && inbytes[1] == 'E' && inbytes[2] == 'Q') {
		...
		// we can extract the current sequence and if it is the same as
		// last sequence, we can initiate a card programing sequence!
		...
		G = G * 4;
		if (S == G) {
			// if the last sequence came in, we have to perform a card programming sequnce.
			MaxSeq = G / 4;
...
extractPCsettings(void)
{
	...
	VNN = PCSettings[3] + 1;
	VPP = PCSettings[4] - 1;
	VPPc = (PCSettings[5] - '0') * 100 +
	       (PCSettings[6] - '0') * 10 +
	       (PCSettings[7] - '0');
	VNNc = (PCSettings[8] - '0') * 100 +
	       (PCSettings[9] - '0') * 10 +
	       (PCSettings[10] - '0');
	...
	TrOd = (PCSettings[12] - '0') * 1000 +
	       (PCSettings[13] - '0') * 100 +
	       (PCSettings[14] - '0') * 10 +
	       (PCSettings[15] - '0');
	Qswitch = (PCSettings[16] - '0') * 100 +
	          (PCSettings[17] - '0') * 10 +
	          (PCSettings[18] - '0');
	if (Qswitch < 250)
		Qswitch = 250;
...
setup()
{
	...
	// main trigger, if CCU confirms a delayed trigger out will be created and counted
	attachInterrupt(digitalPinToInterrupt(TriggerPin), Trigger1Interrupt, RISING);
#if 0
