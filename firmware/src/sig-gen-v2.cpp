...
volatile bool AfterStopPulse = 0; // this will determin is there any trigger
                                  // waiting to be set to verasonics
...
{ // creates 10HZ pulses, it can also, can enable trigger out if needed
	...
	if (F10HzOutState) { // if output is high, we waited for 10 us
		...
		if (PAtoken) { // PA requested
			if (FirstTrigger) {
				myQswitchGeneratorTimer.begin(
				    myQswitchGenerator, 5000);
				myQswitchGeneratorTimer.update(5000);
				QswitchState = 2;
				FirstTrigger = false;
			} else {
				myQswitchGeneratorTimer.begin(
				    myQswitchGenerator, Qswitch);
...
Trigger1Interrup(void)
{
	...
	if (ImgMd) {
		...
		myTriggerOutTimer.begin(
		    myTriggerOut,
		    1); // TrOd generator
		        // ___(Trod)___|--(1us)--|__________(TrOd-5)__
		AfterStopPulse = !STOP;
