...
void signalGeneration(bool f10hz) {
  ...
  if (f10hz) {
    //digitalWrite(pinPdly4TrO, LOW);
    my10HzGeneratorTimer.begin(my10HzGenerator, 1.07300);  // 10Hz generator ___(1.07300us)___|---(10us)---|____(9,999,988.927)
...
void my10HzGenerator(void) {  //creates 10HZ pulses, it can also, can enable trigger out if needed
  switch (Freq10HzState) {
    ...
    case 4:
      digitalWrite(pinHzOut, LOW);
      digitalWrite(pinPdly2, LOW);        //delay 2 will have waveform trigger as 10Hz
      digitalWrite(pinPdly3, LOW);        //delay 3 will have waveform trigger as 10Hz
      //here we will change a bit to zero to stop creating pulses, when Trigger 2 comes in, it will hcange this to one to generate pulses
    ...
    case 6:
      if (STOP == 0 && PAtoken == 1) {
        digitalWrite(pinPdly4TrO, HIGH);    //delay 4 will generate a pule with 500 us delay from the 10Hz pulse, which will be feedback to trigger 1 to created a timed trigger out
